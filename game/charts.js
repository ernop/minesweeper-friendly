...
function trendLinesFor(pairs, todayPairs) {
  return analysisTask('charts', 'trends', { pairs, todayPairs });
}
...
function buildScatter(wins, me, fx, fy, xLabel, yLabel, meLabel, ageInfoOf, opts = {}) {
  ...
  const points = wins.map((s) => ({ x: fx(s), y: fy(s), current: s === me,
    age: opts.neutralDots ? null : ageInfoOf(s) }));
  const model = analysisTask('charts', 'scatter', { points, trimY: opts.trimY });
  list.analysisReady = Promise.all([model, opts.trendLines || Promise.resolve([])])
    .then(([data, lines]) => draw(data, lines));
  list.analysisReady.catch(analysisFailure);
  async function draw({ shown, hiddenCount, hiddenMax, xMin, xMax, yMin, yMax }, lines) {
    const pad = (min, max) => {
      const p = (max - min) * 0.04;
      return [min - p, max + p];
    };
    const [x0, x1] = opts.xDomain || pad(xMin, xMax);
    const [y0, y1] = opts.yDomain || pad(yMin, yMax);
    ...
    const dot = (s, cls, r, opacity) => el('circle', {
      cx: px(s.x).toFixed(1), cy: py(s.y).toFixed(1), r, class: cls,
    ...
    for (const s of shown) {
      if (++drawnPoints % 128 === 0) await yieldAnalysisPresentation();
    ...
    const selectedPoint = shown.find((p) => p.current);
    if (selectedPoint) {
      const meClass = opts.neutralDots ? 'scatter-me average-dot' :
        'scatter-me age-dot-' + selectedPoint.age.unit;
      dot(selectedPoint, meClass, '3.5', '1');
      // Today-rank tag beside the me-dot; flips to the left near the right edge.
      if (meLabel) {
        const meX = px(selectedPoint.x);
        const flipLeft = meX > W - R - 50;
        el('text', {
          x: (flipLeft ? meX - 6 : meX + 6).toFixed(1),
          y: Math.max(T + 9, py(selectedPoint.y) - 5).toFixed(1),
...
function averageScatterData(spec, wins, allRecords, record, historyView, mode, referenceMs) {
  const todayStart = startOfDay(referenceMs);
  const selected = !historyView && averageEligibleWins(spec, [record]).length > 0;
  let points, todayPairs, currentIndex, title, yLabel;
  if (mode === 'winrate') {
    if (spec.winBound) return null;
    const groups = winratePoints(spec, allRecords);
    if (groups.length < 2) return null;
    points = groups.map((p) => ({ x: p.x, y: p.winratePct, endedAt: p.endedAt }));
    currentIndex = selected ? points.findIndex((p) => p.x === spec.value(record)) : -1;
    title = 'winrate by '; yLabel = 'winrate';
  } else {
    const eligible = averageEligibleWins(spec, wins);
    if (eligible.length < 2) return null;
    if (mode === 'distribution') {
      points = eligible.map((w) => ({ x: spec.value(w), y: secondsOf(w), endedAt: w.endedAt }));
      todayPairs = points.filter((p) => p.endedAt >= todayStart).map((p) => [p.x, p.y]);
      currentIndex = selected ? eligible.indexOf(record) : -1;
      title = 'times by '; yLabel = 'time';
    } else {
      points = averagePoints(spec, eligible).map((p) => ({ x: p.x, y: p.averageSeconds, endedAt: p.endedAt }));
      todayPairs = averagePoints(spec, eligible.filter((w) => w.endedAt >= todayStart))
        .map((p) => [p.x, p.averageSeconds]);
      currentIndex = selected ? points.findIndex((p) => p.x === spec.value(record)) : -1;
      title = 'time by '; yLabel = 'average time';
    }
  }
...
function scatterPlotData(points, trimY) {
  let shown = points, hiddenCount = 0, hiddenMax = 0;
  if (trimY && points.length >= 8) {
    const sorted = points.map((p) => p.y).sort((a, b) => a - b);
    const q = (p) => {
      const at = (sorted.length - 1) * p, lo = Math.floor(at);
      return sorted[lo] + (at - lo) * (sorted[Math.min(lo + 1, sorted.length - 1)] - sorted[lo]);
    };
    const fence = q(.75) + 1.5 * (q(.75) - q(.25));
    shown = [];
    for (const point of points) {
      if (point.y <= fence || point.current) shown.push(point);
      else { hiddenCount++; hiddenMax = Math.max(hiddenMax, point.y); }
    }
  }
  ...
  return { shown, hiddenCount, hiddenMax, xMin, xMax, yMin, yMax };
}
